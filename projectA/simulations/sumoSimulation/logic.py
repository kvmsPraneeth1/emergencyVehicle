...
net_file = r'output.net.xml'  # Replace with the path to your net.xml file
sumo_cfg_file = r'output.sumocfg'  # Replace with your SUMO config file
output_graph_file = 'graph_code11.txt'  # Output graph file
flag = True
temp = []
temp2 = []

# Add SUMO tools to system path
if sumo_tools_path not in os.environ['PATH']:
    os.environ['PATH'] += os.pathsep + sumo_tools_path
...
# Loop through all edges in the network
    ...
    # Get the edge ID and length (distance) of the edge
        ...
        density += edge.getLength()
        density /= avgspeed + 1


        print(f"edge ID {edge_id} and density avg + density + len {avgspeed } + {d} + {edge.getLength()} = {density}")
...
def traffic_light_controller(start_node, end_node):
   ...
   while traci.simulation.getMinExpectedNumber() > 0:
        ...
        speed = traci.vehicle.getSpeed("vehicle0")
        temp.append(speed)
...
def start_simulation(path_edges):
    traci_connection=traci.init(port=9999)
    traci.setOrder(2)
    print(traci_connection)
